...
$(document).ready(function () {
    ...
    Apis.getRequest('customer/mypage/${memberId}').then(response =>{
...
function button_myprofile(){
    ...
    if(checkAll() === true){
        ...
        Apis.updatecustomerProfile(customerMypageObject).then(response => { //JSON으로 변환된 값들을 DB로 보낸다.
            Apis.getRequest('customer/mypage/${memberId}').then(response =>{ //updatecustomerProfile함수에서 반환된 값(수정된 mypage 정보)를 다시 뿌려준다.
                console.log(response);//promise객체로 옴. 이제 그걸 풀어서, 화면에 뿌려줘야함.
                showMypage(response);
            });
        }); alert("수정 완료!")
