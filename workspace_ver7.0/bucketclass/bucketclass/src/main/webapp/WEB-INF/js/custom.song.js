...
function firstServiceSave(){
	...
	var serviceTitle = document.getElementById("search-field").value;

	// 카테고리 category_subject
	var categorySubject = document.getElementsByClassName("chosen-select-no-single")[0].value;
	...
	var categoryType = ($(youhyung).prop("checked"))? "체험" : ($(youhyung2).prop("checked"))?"교육":"";

	// category_scale 기간
	var gigan = document.getElementById("check-c1");
	var gigan2 = document.getElementById("check-d1");
	var categoryPeriod = ($(gigan).prop("checked"))? "정기" : ($(gigan2).prop("checked"))?"비정기":"";

	// category_period 규모
	var gyumo = document.getElementById("check-e1");
	var gyumo2 = document.getElementById("check-f1");
	var categoryScale = ($(gyumo).prop("checked"))? "개인" : ($(gyumo2).prop("checked"))?"단체":"";

	// category_place 장소
	var jangso = document.getElementById("check-g1");
	var jangso2 = document.getElementById("check-h1");
	var categoryPlace = ($(jangso).prop("checked"))? "실내" : ($(jangso2).prop("checked"))?"실외":"";
	...
	var serviceDescription = document.getElementById("summary").value;

	// 사진 service_img_uri
	// var serviceImgUri;

	// 계좌은행명 account_bank chosen-select
	var accountBank = document.getElementById("chosen-select").value;
	...
	//
	// sendJson.serviceImgUri = service_img_uri;
	//
	// return sendJson;
